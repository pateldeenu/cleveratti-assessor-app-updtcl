...
export default function JoinChannelVideo({navigation,route}) {
  const {
    batch_id,
    rtcToken,
    appId,
  ...
  const [channelId] = useState(batch_id);
  const [token] = useState(rtcToken);
  ...
  const initRtcEngine = useCallback(async () => {
    ...
    //add these code for save single screen
    const videoConfig = {
      orientationMode: 2, // 1 for fixed landscape, 2 for fixed portrait
    };
    engine.current.setVideoEncoderConfiguration(videoConfig);
  ...
  function renderChannel() {
    return (
      <>
        <View style={AgoraStyle.viewMargin}>
          <MenuIcon onPress={() => navigation.goBack()} back="back" />
          <Text style={AgoraStyle.ques}>{`Batch_id :  ${batch_id_show}`}</Text>
        </View>
      </>
    );
  }

  //show webportal videos
  ...
  function renderVideo(uid) {
    return renderByTextureView ? (
      ...
      // <></>
      <RtcSurfaceView
        // style={uid === 0 ? AgoraStyle.videoLarge : AgoraStyle.videoLarge}
        style={uid === 0 ? AgoraStyle.videoLarge : AgoraStyle.videoSmall}
  ...
  function renderConfiguration() {
    return (
      <>
        ...
        /> */}
        {/* <AgoraButton
          title={`${joinChannelSuccess ? "leave" : "Start"} `}
          onPress={() => {
            joinChannelSuccess ? leaveChannel() : joinChannel();
          }}
        /> */}
